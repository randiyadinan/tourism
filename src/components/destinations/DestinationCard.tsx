    ...
    <div className="group bg-white rounded-xl border border-stone-200/90 overflow-hidden shadow-xs hover:shadow-md transition-all duration-300 flex flex-col justify-between">
      ...
      <div className="relative h-60 overflow-hidden">
        ...
        <div className="absolute top-3 left-3">
          <span className="px-2.5 py-1 rounded-md text-[11px] font-semibold bg-white/90 text-[#0D3B2E] shadow-xs">
        ...
        <div className="absolute bottom-3 left-3 right-3">
          <h3 className="font-serif text-2xl font-bold text-white leading-snug drop-shadow-xs">
            {destination.name}
          </h3>
        </div>
      </div>
      ...
      <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
        ...
        <div className="pt-3 border-t border-stone-100 flex items-center justify-between">
          <span className="text-xs font-semibold text-[#1F6B50]">
            {destination.popularActivities.length} Key Highlights
          </span>
          <Link
            to={`/destinations/${destination.slug}`}
            className="inline-flex items-center gap-1 text-xs font-semibold text-[#0D3B2E] hover:text-[#1F6B50] transition-colors"
          >
            <span>Explore</span>
