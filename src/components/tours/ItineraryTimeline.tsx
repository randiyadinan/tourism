  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h3 className="font-serif text-2xl font-bold text-[#062C22]">Day-by-Day Itinerary</h3>
      ...
      {/* Timeline List */}
      <div className="space-y-4 relative before:absolute before:inset-0 before:left-5 before:w-0.5 before:bg-[#176B52]/30">
        {days.map((day) => {
            ...
            <div key={day.day} className="relative pl-12">
              ...
              <div 
                ...
                className={`absolute left-0 top-3 w-10 h-10 rounded-full flex items-center justify-center font-serif text-sm font-bold cursor-pointer transition-all shadow-md z-10 ${
                  isOpen 
                    ? 'bg-[#0B3D2E] text-[#39A982] ring-4 ring-[#176B52]/20' 
                    : 'bg-white text-stone-700 border-2 border-[#176B52]'
                }`}
              >
                {day.day}
              </div>
              ...
              <div className="bg-white rounded-2xl border border-stone-200 overflow-hidden shadow-sm transition-all">
                <button
                  onClick={() => toggleDay(day.day)}
                  className="w-full p-5 text-left flex items-center justify-between gap-4 hover:bg-[#F8F7F2] transition-colors"
                >
                  <div className="space-y-1">
                    ...
                    <h4 className="font-serif text-lg font-bold text-[#062C22]">{day.title}</h4>
                  </div>
                ...
                {isOpen && (
                  <div className="px-5 pb-6 pt-2 space-y-4 border-t border-stone-100 text-sm text-stone-600 animate-fadeIn">
                    ...
                    {day.highlights && day.highlights.length > 0 && (
                      <div className="bg-[#F8F7F2] p-4 rounded-xl space-y-2 border border-stone-200/60">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                          {day.highlights.map((h, i) => (
                            <div key={i} className="flex items-center gap-2 text-xs text-stone-700">
                              <Check className="w-3.5 h-3.5 text-[#0B3D2E] shrink-0" />
                              <span>{h}</span>
                            </div>
                          ))}
                        </div>
