...
export const TourCard: React.FC<TourCardProps> = ({ tour, layout = 'grid' }) => {
  ...
  if (layout === 'horizontal') {
    return (
      <div className="group bg-white rounded-3xl border border-stone-200/80 overflow-hidden shadow-[0_4px_20px_-4px_rgba(6,44,34,0.05)] hover:shadow-[0_20px_40px_-10px_rgba(6,44,34,0.12)] transition-all duration-300 flex flex-col md:flex-row transform hover:-translate-y-1">
        {/* Image */}
        ...
        <div className="p-6 md:p-8 md:w-3/5 flex flex-col justify-between space-y-4">
          <div className="space-y-2">
            <div className="flex items-center justify-between text-xs">
              <span className="inline-flex items-center gap-1 font-semibold text-[#176B52] bg-[#DDEFE8]/60 px-3 py-1 rounded-full">
      ...
      <div>
        ...
        <div className="relative h-60 overflow-hidden">
          ...
          <div className="absolute bottom-3.5 left-3.5">
            <span className="px-2.5 py-1 rounded-lg text-xs font-semibold bg-white/90 text-[#062C22] backdrop-blur-md shadow-xs">
              {tour.durationDays} Days / {tour.durationNights} Nights
            </span>
          </div>
        </div>
