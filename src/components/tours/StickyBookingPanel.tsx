...
export const StickyBookingPanel: React.FC<StickyBookingPanelProps> = ({ tour, onBookNow }) => {
  ...
  return (
    <div className="bg-white rounded-3xl border border-stone-200/90 p-6 sm:p-7 shadow-[0_10px_30px_-10px_rgba(7,94,99,0.08)] space-y-5">
      ...
      <div className="flex items-baseline justify-between border-b border-stone-100 pb-4">
        <div>
          ...
          <div className="flex items-baseline gap-1.5">
            <span className="font-serif text-3xl font-bold text-[#075E63]">
        ...
        <div className="text-right">
          <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold bg-[#DDF5F0] text-[#075E63]">
      ...
      <div className="space-y-3.5">
        ...
        <div className="space-y-1">
          <label className="text-xs font-semibold text-[#173238] flex items-center gap-1.5">
          ...
          <input
            ...
            className="w-full bg-[#F6F1E7] border border-stone-300 rounded-xl px-3.5 py-2.5 text-xs sm:text-sm font-medium text-[#173238] focus:outline-none focus:ring-2 focus:ring-[#0B7A75]"
          />
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1">
            <label className="text-xs font-semibold text-[#173238]">
              Adults (12+ yrs)
            </label>
            <select
              value={adults}
              onChange={(e) => setAdults(Number(e.target.value))}
              className="w-full bg-[#F6F1E7] border border-stone-300 rounded-xl px-3 py-2 text-xs sm:text-sm font-medium text-[#173238] focus:outline-none focus:ring-2 focus:ring-[#0B7A75]"
            >
        ...
        <label className="flex items-start gap-2.5 p-3 rounded-2xl border border-stone-200 bg-[#F6F1E7] cursor-pointer hover:border-[#0B7A75] transition-colors">
          <input
            type="checkbox"
          ...
          <div className="text-xs">
            <span className="font-semibold text-[#173238] flex items-center gap-1">
              <Plane className="w-3.5 h-3.5 text-[#0B7A75]" />
      ...
      <form onSubmit={handleApplyPromo} className="space-y-1.5 pt-1">
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 bg-[#F6F1E7] border border-stone-300 rounded-xl px-3 py-2 text-xs font-medium text-[#173238] uppercase placeholder-stone-400 focus:outline-none focus:ring-2 focus:ring-[#0B7A75]"
          />
          <button
            type="submit"
        ...
        {appliedDiscount && (
          <div className="flex items-center justify-between text-xs text-[#075E63] bg-[#DDF5F0] px-3 py-1.5 rounded-xl border border-[#0B7A75]/30">
      ...
      {/* Pricing Summary */}
      <div className="bg-[#F6F1E7] rounded-2xl p-4 space-y-2 text-xs border border-stone-200/70">
        <div className="flex justify-between text-[#68736E]">
          <span>{adults} Adult{adults > 1 ? 's' : ''} (${tour.pricePerPerson} ea)</span>
          <span className="font-semibold text-[#173238]">${adultBaseTotal.toLocaleString()}</span>
        </div>

        {children > 0 && (
          <div className="flex justify-between text-[#68736E]">
            <span>{children} Child{children > 1 ? 'ren' : ''} (35% off)</span>
            <span className="font-semibold text-[#173238]">${childBaseTotal.toLocaleString()}</span>
          </div>
        )}
        ...
        <div className="flex justify-between text-sm font-bold text-[#173238] pt-2 border-t border-stone-200">
          <span>Total (USD)</span>
          <span className="font-serif text-xl text-[#075E63]">${finalTotal.toLocaleString()}</span>
        </div>
      </div>
      ...
      <button
        ...
        className="w-full py-3.5 px-4 rounded-2xl bg-[#0B7A75] hover:bg-[#075E63] text-white font-semibold text-sm transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 border border-white/20"
      >
      ...
      <div className="space-y-1.5 text-[11px] text-[#68736E] pt-1">
        ...
        <div className="flex items-center gap-1.5">
          <CreditCard className="w-3.5 h-3.5 text-[#0B7A75]" />
