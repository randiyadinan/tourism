  ...
  if (!activity) {
    return (
      <div className="max-w-4xl mx-auto py-20 px-4 text-center">
        <h2 className="font-serif text-3xl font-bold text-[#062C22] mb-4">Activity Not Found</h2>
        <Link to="/activities" className="px-6 py-3 bg-[#0B3D2E] text-white font-bold rounded-xl">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-stone-200/80 pb-4">
          <div className="flex items-center gap-2 text-xs text-stone-500 font-medium">
            <Link to="/" className="hover:text-[#0B3D2E]">Home</Link>
            <span>/</span>
            <Link to="/activities" className="hover:text-[#0B3D2E]">Activities</Link>
            <span>/</span>
            <span className="text-[#176B52] font-bold">{activity.title}</span>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
          ...
          <div className="lg:col-span-7 space-y-8">
            ...
            <div className="relative h-96 rounded-3xl overflow-hidden shadow-md">
              ...
              />
              <span className="absolute top-4 left-4 px-3 py-1 rounded-full text-xs font-bold bg-[#0B3D2E] text-[#39A982]">
                {activity.category}
              </span>
            </div>

            <div className="space-y-3">
              <h1 className="font-serif text-3xl sm:text-4xl font-bold text-[#062C22]">
                {activity.title}
              </h1>
              <div className="flex items-center gap-4 text-xs font-medium text-stone-600">
                <span className="flex items-center gap-1"><MapPin className="w-3.5 h-3.5 text-[#176B52]" /> {activity.destination}</span>
                <span className="flex items-center gap-1"><Clock className="w-3.5 h-3.5 text-[#176B52]" /> {activity.duration}</span>
            ...
            <div className="bg-white p-6 rounded-3xl border border-stone-200 shadow-sm space-y-3">
              <h3 className="font-serif text-xl font-bold text-[#062C22]">Experience Description</h3>
              <p className="text-stone-600 leading-relaxed text-sm">{activity.description}</p>
            </div>

            {/* Highlights */}
            <div className="bg-white p-6 rounded-3xl border border-stone-200 shadow-sm space-y-3">
              <h3 className="font-serif text-xl font-bold text-[#062C22]">Highlights</h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="bg-white p-6 rounded-3xl border border-stone-200 shadow-sm space-y-3">
                <h4 className="font-serif font-bold text-base text-[#062C22]">What’s Included</h4>
              ...
              <div className="bg-white p-6 rounded-3xl border border-stone-200 shadow-sm space-y-3">
                <h4 className="font-serif font-bold text-base text-[#062C22]">What to Bring</h4>
                <ul className="space-y-2 text-xs text-stone-600">
                  {activity.whatToBring.map((item, i) => (
                    <li key={i} className="flex items-start gap-1.5">
                      <Info className="w-3.5 h-3.5 text-[#176B52] shrink-0 mt-0.5" />
                      <span>{item}</span>
                    </li>
                  ))}
                </ul>
          ...
          <div className="lg:col-span-5">
            <div className="bg-white rounded-3xl border border-stone-200 p-6 sm:p-8 shadow-xl space-y-6 sticky top-24">
              ...
              <div className="border-b border-stone-100 pb-4">
                <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#0B3D2E]/10 text-[#0B3D2E] text-xs font-bold uppercase tracking-wider mb-2">
                  <Sparkles className="w-3.5 h-3.5 text-[#176B52]" />
                  Sri Lanka Experience
                </div>
                <h3 className="font-serif text-2xl font-bold text-[#062C22]">{activity.title}</h3>
                <p className="text-xs text-stone-500 mt-1">{activity.destination} • {activity.duration} • {activity.difficulty} Pace</p>
              </div>

              <div className="bg-[#F8F7F2] p-5 rounded-2xl border border-stone-200/80 space-y-3 text-xs text-stone-600">
                <div className="flex items-center gap-2 font-bold text-[#062C22]">
              ...
              <div className="space-y-3 pt-2">
                <Link
                  ...
                  className="w-full py-4 bg-gradient-to-r from-[#0B3D2E] to-[#134E3F] hover:from-[#134E3F] hover:to-[#0B3D2E] text-white font-bold text-sm rounded-2xl transition-all shadow-lg flex items-center justify-center gap-2"
                >
