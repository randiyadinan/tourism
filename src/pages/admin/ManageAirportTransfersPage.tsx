  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="font-serif text-2xl sm:text-3xl font-bold text-[#062C22]">Airport Transfers Logistics</h1>
      ...
      <div className="bg-white p-4 rounded-2xl border border-stone-200 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
          {/* Status Tabs */}
          <div className="flex items-center gap-1 bg-[#F8F7F2] p-1 rounded-xl border border-stone-200">
          ...
          <select
            ...
            className="px-3 py-2 bg-[#F8F7F2] border border-stone-300 rounded-xl text-xs text-[#062C22] font-semibold"
          >
              ...
              {filtered.map((t) => (
                <tr key={t.id} className="hover:bg-stone-50/50">
                  ...
                  <td className="py-4 px-4">
                    <strong className="text-[#062C22] block">{t.contactName}</strong>
                    <span className="text-[10px] text-stone-400">{t.contactPhone}</span>
                  </td>
                  <td className="py-4 px-4">
                    <div className="flex items-center gap-1 font-semibold text-[#062C22]">
                      <Plane className="w-3 h-3 text-[#176B52]" />
                  ...
                  <td className="py-4 px-4 truncate max-w-[180px]">
                    <div className="font-medium text-[#062C22] truncate">{t.destinationArea}</div>
                    <span className="text-[10px] text-stone-400 truncate block">{t.hotelAddress}</span>
                  </td>
                  <td className="py-4 px-4">
                    <span className="font-medium text-[#062C22] block">{t.vehicleName}</span>
                    <span className="text-[10px] text-stone-400">{t.passengers} pax • {t.luggageCount} bags</span>
                  </td>
                  <td className="py-4 px-4 font-bold text-[#062C22]">${t.totalPriceUSD}</td>
                  <td className="py-4 px-4">
                    <select
                      value={t.status}
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4 backdrop-blur-xs">
          <div className="bg-white w-full max-w-lg rounded-3xl p-6 sm:p-8 shadow-2xl border border-stone-200 space-y-6">
            <div className="flex items-center justify-between border-b border-stone-100 pb-4">
              <div>
                <span className="text-xs font-bold text-[#176B52] block">{selectedTransfer.bookingCode}</span>
                <h3 className="font-serif text-xl font-bold text-[#062C22]">Transfer Booking Details</h3>
              </div>
              <button
            ...
            <div className="space-y-3 text-xs text-stone-600">
              <div className="flex justify-between py-1.5 border-b border-stone-100">
                ...
                <strong className="text-[#062C22]">{selectedTransfer.contactName}</strong>
              </div>
              <div className="flex justify-between py-1.5 border-b border-stone-100">
                <span className="font-medium">Phone & Email:</span>
                <span className="text-[#062C22]">{selectedTransfer.contactPhone} ({selectedTransfer.contactEmail})</span>
              </div>
              <div className="flex justify-between py-1.5 border-b border-stone-100">
                <span className="font-medium">Airport:</span>
                <span className="text-[#062C22]">{selectedTransfer.airport}</span>
              </div>
              <div className="flex justify-between py-1.5 border-b border-stone-100">
                <span className="font-medium">Flight:</span>
                <strong className="text-[#062C22]">{selectedTransfer.flightNumber} ({selectedTransfer.flightDate} at {selectedTransfer.flightTime})</strong>
              </div>
              <div className="flex justify-between py-1.5 border-b border-stone-100">
                <span className="font-medium">Drop-off / Hotel:</span>
                <span className="text-[#062C22] text-right">{selectedTransfer.hotelAddress}</span>
              </div>
              <div className="flex justify-between py-1.5 border-b border-stone-100">
                <span className="font-medium">Vehicle Class:</span>
                <span className="text-[#062C22]">{selectedTransfer.vehicleName}</span>
              </div>
              <div className="flex justify-between py-1.5 border-b border-stone-100">
            ...
            <div className="flex justify-end pt-2">
              <button
                onClick={() => setSelectedTransfer(null)}
                className="px-5 py-2 bg-[#0B3D2E] text-white text-xs font-bold rounded-xl hover:bg-[#134E3F]"
              >
                Close Inspector
              </button>
            </div>
