...
export class AuthService {
    ...
    async getUserById(id) {
        ...
        if (prisma) {
            try {
                ...
                if (dbUser) {
                    this.memoryUsers.set(dbUser.id, dbUser);
                    const { passwordHash: _p, emailVerificationTokenHash: _t, ...safe } = dbUser;
                    return safe;
                }
            }
            catch (err) {
    ...
    async updateUser(id, updates) {
        ...
        if (updates.passportNumber !== undefined)
            user.passportNumber = updates.passportNumber;
        if (updates.dietaryPreferences !== undefined)
            user.dietaryPreferences = updates.dietaryPreferences;
