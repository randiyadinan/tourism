 ...
 */
export function verifyPayHereNotificationHash(merchantId, orderId, payhereAmount, payhereCurrency, statusCode, receivedMd5sig, merchantSecret) {
    if (!receivedMd5sig)
        return false;
    const hashedSecret = md5(merchantSecret.trim()).toUpperCase();
 ...
 */
function getEnvVar(c, key) {
    return (c.env && c.env[key]) || (typeof process !== 'undefined' ? process.env[key] : undefined);
}
/**
 * POST /api/payhere/initiate
 * Generates secure parameters and hash for PayHere Sandbox/Live Checkout
 */
export async function initiatePayment(c) {
    try {
        const body = await c.req.json().catch(() => ({}));
        const { orderId, bookingId, bookingCode, userId, amount, currency = 'LKR', itemTitle, customerName, customerEmail, customerPhone, flightNumber, address = 'No. 12, Galle Road', city = 'Colombo', country = 'Sri Lanka' } = body;
 ...
 */
export async function handleNotification(c) {
    try {
        let payload = {};
        ...
        const { merchant_id, order_id, payment_id, payhere_amount, payhere_currency, status_code, md5sig, method, status_message, card_holder_name, card_no, card_expiry } = payload;
        const merchantSecret = getEnvVar(c, 'PAYHERE_MERCHANT_SECRET');
        if (!merchantSecret) {
        ...
        // Verify signature
        const isValidSignature = verifyPayHereNotificationHash(merchant_id, order_id, payhere_amount, payhere_currency, status_code, md5sig, merchantSecret);
        if (!isValidSignature) {
        ...
        if (mappedStatus === 'SUCCESS') {
            ...
            if (updatedBooking) {
                ...
                }).catch((err) => console.warn('⚠️ [EmailService] Customer payment receipt email warning:', err));
                emailService.sendAdminPaymentNotification({
                    ...
                    amount: Number(payhere_amount || updatedBooking.totalAmount),
                    tourTitle: updatedBooking.tourTitle || 'Sri Lanka Tour'
                }).catch((err) => console.warn('⚠️ [EmailService] Admin payment notification email warning:', err));
            }
        }
 ...
 */
export async function getPaymentStatus(c) {
    try {
