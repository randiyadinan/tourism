...
// ══════════════════════════════════════════════════════════════════════
export async function getAdminUsers(c) {
    try {
...
export async function getAdminUserById(c) {
    try {
...
export async function updateAdminUser(c) {
    try {
        const id = c.req.param('id');
        if (!id)
            return c.json({ success: false, error: 'User ID is required' }, 400);
...
export async function deleteAdminUser(c) {
    try {
        const id = c.req.param('id');
        if (!id)
            return c.json({ success: false, error: 'User ID is required' }, 400);
...
// ══════════════════════════════════════════════════════════════════════
export async function getAdminBookings(c) {
    try {
...
export async function getAdminBookingById(c) {
    try {
        const id = c.req.param('id');
        if (!id)
...
export async function confirmAdminBooking(c) {
    try {
        const id = c.req.param('id');
        if (!id)
            return c.json({ success: false, error: 'Booking ID is required' }, 400);
...
export async function rejectAdminBooking(c) {
    try {
        const id = c.req.param('id');
        if (!id)
            return c.json({ success: false, error: 'Booking ID is required' }, 400);
...
export async function updateAdminBooking(c) {
    try {
        const id = c.req.param('id');
        if (!id)
            return c.json({ success: false, error: 'Booking ID is required' }, 400);
...
export async function deleteAdminBooking(c) {
    try {
        const id = c.req.param('id');
        if (!id)
            return c.json({ success: false, error: 'Booking ID is required' }, 400);
...
// ══════════════════════════════════════════════════════════════════════
export async function getAdminTours(c) {
    try {
...
export async function getAdminTourById(c) {
    try {
        const id = c.req.param('id');
        if (!id)
            return c.json({ success: false, error: 'Tour ID is required' }, 400);
...
export async function createAdminTour(c) {
    try {
        const body = await c.req.json().catch(() => ({}));
...
export async function updateAdminTour(c) {
    try {
        const id = c.req.param('id');
        if (!id)
            return c.json({ success: false, error: 'Tour ID is required' }, 400);
...
export async function deleteAdminTour(c) {
    try {
        const id = c.req.param('id');
        if (!id)
            return c.json({ success: false, error: 'Tour ID is required' }, 400);
...
// ══════════════════════════════════════════════════════════════════════
export async function getAdminDestinations(c) {
    try {
...
export async function getAdminDestinationById(c) {
    try {
        const id = c.req.param('id');
        if (!id)
            return c.json({ success: false, error: 'Destination ID is required' }, 400);
...
export async function createAdminDestination(c) {
    try {
        const body = await c.req.json().catch(() => ({}));
...
export async function updateAdminDestination(c) {
    try {
        const id = c.req.param('id');
        if (!id)
            return c.json({ success: false, error: 'Destination ID is required' }, 400);
...
export async function deleteAdminDestination(c) {
    try {
        const id = c.req.param('id');
        if (!id)
            return c.json({ success: false, error: 'Destination ID is required' }, 400);
...
// ══════════════════════════════════════════════════════════════════════
export async function getAdminReports(c) {
    try {
        ...
        const filteredBookings = allBookings.filter(b => {
            ...
            if (filterStart && bDate < filterStart)
                return false;
            if (filterEnd && bDate > filterEnd)
                return false;
            return true;
        });
        // 1. BUSINESS OVERVIEW
        ...
        for (const b of allBookings) {
            ...
            if (b.bookingStatus === 'Pending')
                monthlySummaryMap[monthKey].pending += 1;
            ...
            if (b.bookingStatus === 'Rejected' || b.bookingStatus === 'Cancelled')
                monthlySummaryMap[monthKey].rejected += 1;
